...
func (c *Camera) Update(dTime float64) {
...
func (c *Camera) updatePosition(dTime float64) {
	adjustedSpeed := float32(dTime * c.moveSpeed)

	if c.inputManager.IsActive(window.USER_FORWARD) {
		c.pos = c.pos.Add(c.front.Mul(adjustedSpeed))
	}
	if c.inputManager.IsActive(window.USER_BACKWARD) {
		c.pos = c.pos.Sub(c.front.Mul(adjustedSpeed))
	}
	if c.inputManager.IsActive(window.USER_LEFT) {
		c.pos = c.pos.Sub(c.front.Cross(c.up).Normalize().Mul(adjustedSpeed))
	}
	if c.inputManager.IsActive(window.USER_RIGHT) {
